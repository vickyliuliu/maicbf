...
def main():
    ...
    for istep in range(config.EVALUATE_STEPS):
        ...
        if args.vis > 0:
            ...
            print("length of traj:", s_traj_ours.shape[0])
            ...
            for j in range(0, s_traj_ours.shape[0], 10):
                ...
                for k in range(scene.num_goals_per_agent):
                    ax_1.scatter(scene.end_points[:, k, 0], scene.end_points[:, k, 1], scene.end_points[:, k, 2],
