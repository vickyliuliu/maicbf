...
class Cityscape(object):
    ...
    def reset_end_points(self):
        num_goals_per_agent = self.num_goals_per_agent
        end_points = np.zeros((self.num_agents, num_goals_per_agent, 3), dtype=np.float32)
        for k in range(num_goals_per_agent):
            for i in range(self.num_agents):
                ...
                if k == 0:
                    ...
                    dist = min(np.amin(np.linalg.norm(
                            self.obstacle_points - random_end[0, :2], axis=1)),
                            np.linalg.norm(
                                end_points[i, k-1, :2] - random_end[0, :2]) / 5)
                while dist < 1:
                    random_end = np.random.randint(
                        low=1, high=self.area_size-1, size=(1, 3))
                    random_end[0, 2] = min(6, random_end[0, 2])
    ...
    def reset_reference_paths(self):
        ...
        if self.show:
            plt.ion()
            fig = plt.figure()
        for k in range(self.num_goals_per_agent):
            reference_paths_per_round = []
            for i in range(self.num_agents):
                sx, sy, sz = self.start_points[i] if k == 0 else self.end_points[i, k-1]
                gx, gy, gz = self.end_points[i, k]
                ...
                reference_paths_per_round.append(path)
                # max_time = max(max_time, rx.shape[0])
                time_stamps[i] += rx.shape[0]
                if self.show:
                    plt.clf()
                    ...
                    fig.canvas.draw()
                    time.sleep(1)
            reference_paths.append(reference_paths_per_round)
        
        max_time = max(time_stamps.values())
        waypoints = []
        for i in range(self.num_agents):
            path_extend = np.zeros(shape=(max_time, 3), dtype=np.float32)
            acc_length = 0
            for k in range(self.num_goals_per_agent):
                path = reference_paths[k][i]
                path_extend[acc_length:acc_length+path.shape[0]] = path
                if k == self.num_goals_per_agent - 1:
                    path_extend[acc_length+path.shape[0]:] = path[-1]
                acc_length += path.shape[0]
